...
class TestNGTestClassProcessorTest extends Specification {
    ...
    void "custom test listeners are registered before Gradle's listener"() {
        ...
        // 2 times because addListener(Object) calls addListener(ITestNGListener)
        then: 2 * testNG.addListener({ it instanceof CustomTestListener })
        then: 2 * testNG.addListener({ Proxy.isProxyClass(it.class) && it.toString().contains(TestNGTestResultProcessorAdapter.class.name) })
    }
